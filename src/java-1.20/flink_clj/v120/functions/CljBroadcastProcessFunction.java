...
/**
 * Flink BroadcastProcessFunction that delegates to Clojure functions.
 *
 * Handles non-keyed streams with broadcast state. The process-element function
 * has read-only access to broadcast state, while process-broadcast can modify it.
 */
public class CljBroadcastProcessFunction<IN1, IN2, OUT>
    ...
    public void processElement(IN1 value, ReadOnlyContext ctx, Collector<OUT> out) throws Exception {
        if (processFn != null) {
            // Create a wrapper around the read-only broadcast state
    ...
    public void processBroadcastElement(IN2 value, Context ctx, Collector<OUT> out) throws Exception {
        if (broadcastFn != null) {
            // Create a wrapper around the mutable broadcast state
            IPersistentMap ctxMap = PersistentHashMap.create(
                elementKw, value,
    ...
    /**
     * Wrapper for read-only broadcast state access from processElement.
     */
    ...
    /**
     * Wrapper for mutable broadcast state access from processBroadcastElement.
     */
